...
var OrganizationService = require('../services/organizationServices').OrganizationService;
var organizationService = new OrganizationService();
var ProjectService = require('../services/projectServices').ProjectService;
var projectService = new ProjectService();
var NotificationService = require('../services/notificationServices').NotificationService;
var notificationService = new NotificationService();
var SprintService = require('../services/sprintServices').SprintService;
var sprintService = new SprintService();
var RolePermissionMappingService = require('../services/rolePermissionMappingServices').RolePermissionMappingService;
    ...
    switch (roles.roles[role[0].name]) {
        ...
        case 2:
            let orgId = objMethods.GetFirstOrDefault(
                await organizationService.GetOrganizationByUserId(userId)).dataValues.id;
        ...
        default:
            ...
            let sprints = await sprintService.GetSprintsByProjectId(projects.map(x => x.id));
...
const JoinOrganization = async function (email, orgId) {
    let userId = objMethods.GetFirstOrDefault(objMethods.GetFirstOrDefault(
        await userService.GetUserIdByEmail(email))).dataValues.id;
    console.log(await organizationService.GetAdminId(orgId));
    let adminId = objMethods.GetFirstOrDefault(
        await organizationService.GetAdminId(orgId)).dataValues.adminId;
    return await notificationService.AddNotification({ adminId: adminId, userId: userId });
}

const AddOrganization = async function (email, req) {
    let userId =
        objMethods.GetFirstOrDefault(objMethods.GetFirstOrDefault
    ...
    if (role[0].dataValues.type !== roles.roles["admin"]) {
        if (await organizationService.AddOrganization(req, userId)) {
            ...
            return status ? await rolePermissionMappingService.UpdateRolePermissionMapping(
                {
